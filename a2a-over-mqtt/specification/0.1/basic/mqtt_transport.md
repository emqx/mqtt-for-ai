...
8. Requesters **MUST** use the same `Task.id` for all subsequent task operations (for example `tasks/get`, `tasks/cancel`, and subscriptions).
...
2. **Multiple in-flight requests per task.** Over one task session a requester may send several concurrent requests (for example `message/send` followed by `tasks/get`). Each needs its own `Correlation Data` so replies can be demultiplexed independently.
...
1. Requester generates a UUIDv4 `Task.id` and publishes the initial `message/send` request (`SendMessageRequest`) with both `Message.task_id` (in the payload) and `Correlation Data` (MQTT property).
...
1. **New conversation.** Requester generates a new `Task.context_id` (UUIDv4) and a new `Task.id` (UUIDv4), publishes `message/send` with fresh `Correlation Data`.
...
3. **Continue interrupted task.** When a responder transitions a task to `input-required` or `auth-required`, the requester publishes a new `message/send` with the **same** `Task.id` and `Task.context_id` and fresh `Correlation Data`. The responder resumes processing on the new correlation. See [Streaming Reply Mapping](#streaming-reply-mapping-messagestream) for stream-final semantics of interrupted states.
...
8. If stream progress stalls longer than `stream_idle_timeout_ms` after at least one stream item, requesters **SHOULD** recover using task follow-up (`tasks/get`) with the known `Task.id` instead of republishing the original request.
...
8. If a requester does not receive terminal status within its stream timeout policy, it **MAY** issue follow-up task retrieval (`tasks/get`) using `Task.id`.

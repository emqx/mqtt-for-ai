...
| `Task.id` | Logical task / conversation session | Responder (on task creation) | Entire task lifecycle |

Both are necessary because:

1. **No Task.id on first request.** When a requester initiates a new task, it does not yet have a `Task.id`. The responder should not respect a requester-supplied task identifier for new sessions. `Correlation Data` is the sole mechanism to match the broker-delivered reply back to the originating publish.
2. **Multiple in-flight requests per task.** Over one task session a requester may send several concurrent requests (for example `tasks/send` followed by `tasks/get`). Each needs its own `Correlation Data` so replies can be demultiplexed independently.
...
1. Requester publishes initial request with `Correlation Data` only (no `Task.id`).
2. Responder creates the task, generates `Task.id`, and returns it in the reply (echoing `Correlation Data`).
3. Requester persists `Task.id` and uses it for all subsequent task operations, generating fresh `Correlation Data` for each new publish.
...
3. For a single logical operation retried multiple times, requesters **MUST** generate new MQTT `Correlation Data` for each publish attempt.
4. Once both parties have an established `Task.id`, requesters **MUST** include it in retry payloads so responders can deduplicate. For the initial task-creating request (before any `Task.id` exists), see **Open Questions § Initial-Request Retry Deduplication**.
...
### Initial-Request Retry Deduplication

Once both parties share a `Task.id`, retry deduplication is straightforward — the requester includes `Task.id` and the responder matches against existing state. The unresolved case is the **initial task-creating request**: the requester has no `Task.id` yet, and if the first attempt is processed but the reply is lost, a retry could create a duplicate task.

**Option A: JSON-RPC request `id`.**
Keep the JSON-RPC envelope `id` stable across retries. Responders track recently seen `id` values to deduplicate. Simple, but overloads JSON-RPC `id` beyond its standard request/response-correlation role (which MQTT `Correlation Data` already fills).

**Option B: Requester-proposed `Task.id`.**
Let the requester propose a `Task.id` on the initial `tasks/send`; the responder accepts or rejects it. The proposed `id` doubles as the dedup key. Aligns with standard A2A where clients provide `Task.id`, but changes this binding's current server-generated `Task.id` rule.

**Option C: Explicit idempotency key.**
Introduce a dedicated MQTT User Property (for example `a2a-idempotency-key`) that the requester holds stable across retries of the same initial request. Cleanly separates dedup from both transport correlation and task identity, at the cost of an additional property.

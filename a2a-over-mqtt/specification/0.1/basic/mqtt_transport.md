...
6. For new tasks, requesters **MUST** generate a `Task.id` as a UUIDv4 value and include it in the request payload.
7. Responders **MUST** use the requester-provided `Task.id` to track task state and **MUST** echo it in all replies and stream items for that task.
...
| `Task.id` | Logical task / conversation session | Requester (UUIDv4, on task creation) | Entire task lifecycle |
...
1. Requester generates a UUIDv4 `Task.id` and publishes the initial request with both `Task.id` (in the payload) and `Correlation Data` (MQTT property).
